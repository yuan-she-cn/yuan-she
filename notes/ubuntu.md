...
# 下载安装包（arm_aarch64）
wget https://download.docker.com/linux/ubuntu/dists/noble/pool/stable/arm64/containerd.io_1.7.28-1~ubuntu.24.04~noble_arm64.deb
wget https://download.docker.com/linux/ubuntu/dists/noble/pool/stable/arm64/docker-ce_28.5.1-1~ubuntu.24.04~noble_arm64.deb
wget https://download.docker.com/linux/ubuntu/dists/noble/pool/stable/arm64/docker-ce-cli_28.5.1-1~ubuntu.24.04~noble_arm64.deb
wget https://download.docker.com/linux/ubuntu/dists/noble/pool/stable/arm64/docker-buildx-plugin_0.29.1-1~ubuntu.24.04~noble_arm64.deb
wget https://download.docker.com/linux/ubuntu/dists/noble/pool/stable/arm64/docker-compose-plugin_2.40.3-1~ubuntu.24.04~noble_arm64.deb
# 安装依赖包
...
# 安装 Docker（arm_aarch64）
sudo dpkg -i ./containerd.io_1.7.28-1~ubuntu.24.04~noble_arm64.deb ./docker-ce_28.5.1-1~ubuntu.24.04~noble_arm64.deb ./docker-ce-cli_28.5.1-1~ubuntu.24.04~noble_arm64.deb ./docker-buildx-plugin_0.29.1-1~ubuntu.24.04~noble_arm64.deb ./docker-compose-plugin_2.40.3-1~ubuntu.24.04~noble_arm64.deb
...
# 查看镜像标签(版本)
skopeo list-tags docker://docker.1ms.run/postgres >> postgresql.tag.txt
...
# 如果 BIOS 开启 Secure Boot 请按以下操作
# 选择 <OK>
# 输入临时密码，选择 <OK>
# 再次临时输入密码，选择 <OK>
sudo dpkg --configure -a
sudo reboot
# 如果 BIOS 开启 Secure Boot 重启后请按以下操作
...
```bash shell
# 检查 CPU 是否支持虚拟化（ > 0 表示支持）
egrep -c "vmx|svm" /proc/cpuinfo
# 检查 KVM 是否可用（输出 KVM acceleration can be used 表示可用，如果不可用可尝试进 BIOS 开启 Intel VT-x 或 AMD-V）
...
1. 打开 virt-manager（虚拟系统管理器）
2. 文件 -> 添加连接
3. 虚拟机管理程序：QEMU/KVM 用户会话
4. 连接
5. 文件 -> 新建虚拟机
6. Forward -> Forward -> Forward -> Forward -> Forward -> 完成

### 安装 ARM 架构系统

以 ubuntu-24.04.4-live-server-arm64.iso 为例

1. 文件 -> 新建虚拟机
2. 选择如何安装操作系统：本地安装介质(ISO 映像或者光驱) -> 架构选项 -> 架构：aarch64 -> 机器类型：virt -> Forward
3. 请选择 ISO 或 CDROM 安装介质：[ubuntu-24.04.4-live-server-arm64.iso 绝对路径] -> 勾选 [自动从安装介质/源检测]（如果检测失败手动输入：Ubuntu 24.04 LTS）-> Forward
4. 设置 [内存] 和 [CPU 数] -> Forward
5. 勾选 [为虚拟机启用存储] -> 勾选 [为虚拟机创建磁盘镜像] -> 设置 [磁盘] -> Forward
